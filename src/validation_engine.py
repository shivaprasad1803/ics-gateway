...
def build_water_tank_engine() -> ValidationEngine:
    """
    Build the standard ValidationEngine for the water tank PLC (PLC_01).
    ...
    Use load_rules_from_yaml("config/rules.yaml") to load all 5 rules from
    config, or register them manually:

        engine = build_water_tank_engine()
        engine.register_rule(AuthRule(allowed_ips={"127.0.0.1"}))
        engine.register_rule(TimeRule(allowed_hours=(8, 18)))

    To add consequence prediction (novel contribution #1):
        engine.set_consequence_engine(ConsequenceEngine())
    ...
    # Any value outside this range is either protocol error or T0855 attack.
    ...
    # Physical justification: based on IEC 61511 safe valve actuation rates
    # for this class of control valve. Rapid changes cause water hammer and
    # pipe stress. 5 %/s → 20 s to traverse full range — physically safe.
    ...
    # Physical justification: pump dry-run (running without sufficient fluid)
    # causes bearing failure and motor burnout within seconds.
    ...
    # M3 fix: wire ConsequenceEngine inside the factory so every call site
    # (tests, modbus_server, future modules) gets consequence metadata by
    # default.  Keeps the factory as the single authoritative wiring point
    # for the full production stack — no caller needs a separate
    # set_consequence_engine() call unless they want a custom CE instance.
    from consequence_engine import ConsequenceEngine
    engine.set_consequence_engine(ConsequenceEngine())

    # H2 fix: register R006 TemporalRule — Novel Contribution #3.
    # Detects slow-drip attacks that stay below R002's per-command rate
    # limit by tracking cumulative valve movement within a 300 s window.
    # Window=300 s, max_cumulative_delta=15% catches the A10 scenario:
    # valve +1%/10s is blocked after 160 s (340 s before overflow at 500 s).
    ...
    # H3 fix: register R007 TopologyRule — Novel Contribution #4.
    # Detects lateral movement attacks (A15) where a command claims to
    # originate from a PLC that is not authorised to target another PLC.
    # default_target="PLC_01" covers the single-PLC deployment: any command
    # without an explicit target_plc_id is assumed to target the water tank
    # PLC, which is authorised (self-path always allowed).
    ...
    # H4 fix: register R008 ReplayRule — detects command replay attacks (A08).
    # Blocks any (address, value) pair that was accepted within the last 5 s.
    # Priority=12: runs after R001 range (10) and before R005 time (15) so
    # a replayed command is identified as A08 before any physics rule fires.
    # Guards both valve (addr=1) and pump (addr=2) with one None-address rule.
    from src.rules.replay_rule import ReplayRule
    engine.register_rule(ReplayRule(
        address=None,
        replay_window_s=5.0,
    ))
    # R009: valve oscillation detector — Novel Contribution (A09, MITRE T0855)
    # Detects back-and-forth setpoint attacks that evade R002 (per-command rate)
    # and R006 (cumulative drift) by tracking direction reversals in a 120s window.
