...
class TestA09_SetpointOscillation:
    ...
    def test_A09_oscillation_blocked_at_max_reversals(self) -> None:
        ...
        # Act — feed 5 alternating commands (builds up 3 reversals)
        for v in [20.0, 80.0, 20.0, 80.0, 20.0]:
            r = rule.evaluate(address=1, value=v, context={}, now=time.monotonic())
            assert r.allowed, f"Setup step val={v} must be allowed"

        # 6th command creates the 4th reversal → BLOCK
        result = rule.evaluate(address=1, value=80.0, context={}, now=time.monotonic())
    ...
    def test_A09_oscillation_caught_in_full_engine(self) -> None:
        ...
        """
        # 1. Arrange - Build the standard engine
        engine = build_water_tank_engine()

        # 2. Clear known rule IDs that might conflict
        # Using a simple list to avoid accessing private engine attributes
        for rid in ["R009", "R006", "A09", "ROSC"]:
            try:
                engine.unregister_rule(rid)
            except (ValueError, KeyError, AttributeError):
                pass

        # 3. Register our specific OscillationRule
        # We manually assign the rule_id so the engine knows it as "R009"
        test_rule = OscillationRule(
            address=1,
            window_s=200.0,
            max_reversals=4,
            min_delta_pct=10.0
        ...
        # If your OscillationRule class has a setter for rule_id, use it.
        # Otherwise, we rely on the engine's registration behavior.
        engine.register_rule(test_rule)

        t0 = 1000.0
        # 20 -> 80 (R1) -> 20 (R2) -> 80 (R3) -> 20 (R4 - Trigger) -> 80 (R5 - Guaranteed)
        seq = [20.0, 80.0, 20.0, 80.0, 20.0, 80.0]
        
        last_result = None

        # 4. Act: Process the sequence through the engine
        for i, val in enumerate(seq):
            ctx = {
                "valve_position": seq[i - 1] if i > 0 else val,
                "tank_level": 50.0,
            }
            # Simulate 20s steps
            res = engine.validate(address=1, value=val, context=ctx, now=t0 + (i * 20))
            
            # Record result and stop if we are blocked
            last_result = res
            if not res.allowed:
                break

        # 5. Assert: The engine must eventually block the oscillation
        assert last_result is not None
        assert not last_result.allowed, (
            f"Engine allowed sequence. History: {getattr(test_rule, 'history', 'N/A')}"
        )
        # Verify it was our oscillation rule that caught it
        assert last_result.rule_id == "R009"
